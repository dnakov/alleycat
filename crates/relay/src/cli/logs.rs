//! `alleycat logs` — read daemon log files directly from disk. The CLI does
//! not go through the daemon; the log file is just a file under
//! `paths::log_dir()`. `--follow` polls at 200ms; we deliberately avoid the
//! `notify` crate to keep the dep tree small.
