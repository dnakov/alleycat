...
//! the output to stderr so `alleycat run` is debuggable from a terminal.
//!
...
///
/// `level` is appended to a baseline `quinn=warn` filter so noisy QUIC
/// internals stay quiet by default. `RUST_LOG` overrides everything when set.
pub fn init(level: &str, log_dir: &Path) -> anyhow::Result<WorkerGuard> {
    ...
    let env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new(format!("{level},quinn=warn")));
