...
use crate::translate::tool::tool_part_to_item;
...
pub async fn route_event(rc: RouteContext<'_>, event: Value) {
    ...
    match event_type {
        "session.status" => {
            let status = props.get("status").cloned().unwrap_or(json!("idle"));
        ...
        "session.idle" => {
            if rc.state.take_active_turn(&thread_id).is_some() {
                let _ = rc.conn.notifier().send_notification(
                    "turn/completed",
                    json!({"threadId":thread_id,"turn":{"id":turn_id,"items":[],"status":"completed"}}),
                );
            }
        }
        "message.updated" => {
...
fn handle_permission_asked(
    ...
    let (method, params) = match kind {
        approval::PermissionKind::Command => {
            ...
            let cwd = metadata.get("cwd").and_then(Value::as_str).map(str::to_owned);
            let mut params = json!({
                "threadId": thread_id,
                "turnId": turn_id,
                "itemId": request_id,
    ...
    if completed_at.is_some() {
        ...
        let _ = rc.conn.notifier().send_notification(
            ...
            json!({
                "threadId": thread_id,
                "turnId": turn_id,
                "item": {"type":"agentMessage","id":message_id,"text":text},
            }),
        );
        rc.state.update_active_turn(thread_id, |turn| {
...
fn handle_message_part_updated(
    ...
    if part_type != "tool" {
        // Text/reasoning/etc. — no per-part item lifecycle; the message-level
        // `item/started` (handle_message_updated) and `item/completed` cover
        // the assistant message as a single AgentMessage item.
        return;
    }
...
async fn handle_session_updated(rc: RouteContext<'_>, props: &Value, thread_id: &str) {
    ...
    if changed
        && let Err(error) = rc.index.insert(binding).await
    {
...
fn emit_token_usage(rc: RouteContext<'_>, thread_id: &str, turn_id: &str, part: &Value) {
    ...
    let _ = rc
        .conn
        .notifier()
        .send_notification("thread/tokenUsage/updated", token_usage_payload(thread_id, turn_id, &snapshot));
}
...
mod tests {
    ...
    fn deprecation_notice_falls_back_when_version_missing() {
        ...
        assert_eq!(
            payload["message"], "An update is available for opencode."
        );
    }
    ...
    fn answers_in_question_order_reorders_by_index() {
        ...
        assert_eq!(ordered, vec![vec!["a".to_string()], vec!["b1".to_string(), "b2".to_string()], Vec::<String>::new()]);
    }
